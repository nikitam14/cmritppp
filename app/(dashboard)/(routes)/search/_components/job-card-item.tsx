...
import { BookmarkCheck, BriefcaseBusiness, Currency, Layers, Loader2 } from "lucide-react";
...
const JobCardItem = ({ job, userId }: JobCardItemProps) => {
  // console.log("JobCardItem job:", job);
        ...
        <div className="w-full h-full p-4 flex flex-col items-start justify-start gap-y-4">
          ...
          <Box className="w-full items-center justify-start gap-x-4">
              ...
              <div className="w-full">
                ...
                <Link href={`/company/${company?.id}`} className="text-xs text-green-500 w-full truncate">
                  {company?.name}
                </Link>
              </div>
          ...
          <Box className="w-full gap-2 mt-auto">
            <Link href={`/search/${job.id}`} className="w-full">
              <Button className="w-full border-green-500 text-green-500 hover:bg-transparent hover:text-green-600"  variant={"outline"}>
                Details
              </Button>
            </Link>

             <Button className="w-full  text-white hover:bg-green-800 bg-green-800/90"  variant={"outline"}>
                Save
              </Button>
          </Box>
