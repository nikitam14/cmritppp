...
export const ShortDescription = ({ initialData, jobId }: ShortDescriptionProps) => {
  ...
  const handlePromptGeneration = async () => {
    try {
      setIsPrompting(true);
      const customPrompt= `Could you craft a concise description for a ${prompt} position in fewer than 400 character?`;
  ...
  return (
    <div className="mt-6 border bg-neutral-100 rounded-md p-4">
      ...
      {!isEditing && (
        <p className="test-neutral-500">{initialData?.short_description}</p>
      )}           
      ...
      {isEditing && <>
        ...
        <p className="text-xs text-muted-foreground text-right">Note*: Name alone enough to generate the tags</p>
        <Form {...form}>
          <form
          ...
          >
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Textarea disabled={isSubmitting} placeholder="Short description about the company" {...field}/>
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
